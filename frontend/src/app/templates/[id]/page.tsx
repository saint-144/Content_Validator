...
function FileRow({ tf, onDelete }: { tf: TemplateFile; onDelete: () => void }) {
  ...
  return (
    <>
      <tr>
        <td><span style={{ fontSize: 16 }}>{tf.file_type === 'video' ? '🎬' : '🖼️'}</span></td>
        ...
        <td style={{ maxWidth: 280, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: '#94a3b8', fontSize: 12 }}>
          {tf.llm_summary?.startsWith('Mock analysis')
            ? <span style={{ color: '#f59e0b' }}>⚠️ Mock data — needs retraining</span>
        ...
        <td>
          <div style={{ display: 'flex', gap: 6 }}>
            {tf.llm_summary && !tf.llm_summary.startsWith('Mock analysis') && (
            ...
            <button onClick={onDelete}
              style={{
                background: 'transparent', border: '1px solid #374151', color: '#ef4444',
                padding: '4px 8px', borderRadius: 5, fontSize: 11, cursor: 'pointer'
              }}>✕</button>
          </div>
        </td>
      </tr>
      {expanded && tf.llm_summary && !tf.llm_summary.startsWith('Mock analysis') && (
        <tr>
              ...
              <div>
                {tf.detected_text && (
                  <>
                    ...
                    <div style={{
                      color: '#d1d5db', fontFamily: 'monospace', background: '#161b27',
...
export default function TemplateDetailPage() {
  ...
  const handleResume = async () => {
    setResuming(true); setMsg(''); setError('');
    try {
      const res = await api.post(`/api/templates/${id}/resume`).then(r => r.data);
      if (res.queued === 0) {
        setMsg('✓ No pending or stuck files found — training is complete');
      } else {
        setMsg(`✓ Resumed training for ${res.queued} file(s) — ${res.unstuck} unstuck, ${res.pending} pending`);
      }
      load();
    } catch (e: any) { setError(e.response?.data?.detail || 'Resume failed'); }
  ...
  const mockCount = template.files.filter(f => f.llm_summary?.startsWith('Mock analysis')).length;
    ...
    <div style={{ padding: 28 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: '#94a3b8', marginTop: 4 }}>
            {template.file_count} files · {doneCount} trained · {processingCount} in queue
            {mockCount > 0 && (
              <span style={{ color: '#f59e0b', marginLeft: 8 }}>· ⚠️ {mockCount} with mock data</span>
            )}
          </div>
        </div>
      ...
      {mockCount > 0 && (
        <div style={{
          ...
          color: '#f59e0b', padding: '10px 14px', borderRadius: 8, fontSize: 13, marginBottom: 14,
          display: 'flex', justifyContent: 'space-between', alignItems: 'center'
        }}>
          <span>⚠️ {mockCount} file(s) have mock LLM data and need retraining. Use Retrain All to fix them.</span>
        </div>
      )}
